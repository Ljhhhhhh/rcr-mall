      ...
      <div class="scroll-content">
        ...
        <div class="car-info_box">
          <div class="car-info_title">
            <span>2016款 锐酷版 外观奥夫特黑+内饰黑色</span>
            <h-tag tag="可配司机" class="tag" :style="{background: '#999999',color:'#FFF'}" />
          </div>
          <div class="car-info_conf">
            <span v-for="(conf, index) in confList" :key="index">{{conf}}</span>
          </div>
          <p class="car-info_offers">
            <span v-for="(offer, index) in offerList" :key="index">{{offer}}</span>
            <label>浙A***88</label>
          </p>
          <div class="car-info_price">
            <p>￥<strong>218</strong>日均<span>(市场价288元)</span></p>
            <p>￥<strong>48</strong>/小时</p>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async getCarDetail() {
      let data = await carDetail(this.carId).data;
      console.log(data);
