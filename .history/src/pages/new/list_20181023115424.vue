      ...
      <transition name="topin" v-show="currentOpenSelect>-1">
        <div class="orderby-select_option" ref="options">
            ...
            <ul class="orderby-select_first" v-show="currentOpenSelect === 2">
              <li v-for="(option, i) in selectList[2].options" :key="i">{{option.tag}}</li>
            </ul>
            <ul class="orderby-select_price" v-show="currentOpenSelect === 3">
              <li v-for="(option, i) in selectList[3].options" :key="i">{{option.tag}}</li>
            </ul>
        </div>
      </transition>
    </div>
...
export default {
  ...
  methods: {
    optionDefault(selectIndex, optionIndex) {
      console.log('321:', this.selectList[selectIndex]);
