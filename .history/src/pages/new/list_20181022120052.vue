<template>
  <div class="container" ref="container">
    ...
    <div class="orderby-select_box van-hairline--bottom" ref="select">
      <span v-for="(select, index) in selectList" :key="index" :class="{'selected': index===currentOpenSelect}" @click="changeSelect(index)">
        <em>{{select.name}}</em>
        <i class="triangle-down_icon"  :class="{'turn-up': index===currentOpenSelect}"></i>
      </span>
      <transition name="topin">
        <div class="orderby-select_option" v-show="currentOpenSelect>-1">
          <template v-for="(select, index) in selectList">
            <ul :class="selectList[currentOpenSelect].class" v-if="index===currentOpenSelect && selectList[currentOpenSelect].name!=='品牌'" :key="index">
              <li v-for="(option, i) in currentOptions" :key="i" :class="{selected:index===option.active}" class="van-hairline--bottom">
                <span>{{option.tag || option.name}}</span>
                <van-icon color="#0198E7" name="success" v-if="index===option.active" />
              </li>
            </ul>
            <div class="brandScrollWrap" v-if="index===currentOpenSelect && selectList[currentOpenSelect].name==='品牌'" :key="index">
    ...
    <transition name="topin">
      <div class="city-comp-box" v-show="cityObj.show" ref="brandBox">
...
export default {
  ...
  computed: {
    currentOptions() {
      return this.selectList[this.currentOpenSelect].options;
    },
  },
  methods: {
    changeSelect(index) {
    ...
    async getBrandList() {
      ...
      let selectHeight = this.$refs.select.offsetHeight;
      this.$refs.brandBox.style['max-height'] =  boxHeight - searchHeight - selectHeight + "px"
      // let brandBoxHeight = this.$refs.container.offsetHeight;
      // container
      // search
      // select
      // brandBox
