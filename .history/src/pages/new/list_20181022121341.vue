        ...
        <div class="orderby-select_option" v-show="currentOpenSelect>-1">
          <template v-for="(select, index) in selectList">
            <ul :class="selectList[currentOpenSelect].class" v-if="index===currentOpenSelect && selectList[currentOpenSelect].name!=='品牌'" :key="index">
              <li v-for="(option, i) in currentOptions" :key="i" :class="{selected:i===option.active}" class="van-hairline--bottom">
                <span>{{option.tag || option.name}}</span>
                <van-icon color="#0198E7" name="success" v-if="i===option.active" />
              </li>
            </ul>
            <div class="brandScrollWrap" ref="brandBox" v-if="index===currentOpenSelect && selectList[currentOpenSelect].name==='品牌'" :key="index">
...
export default {
  ...
  methods: {
    ...
    async getBrandList() {
      ...
      console.log(this.$refs.brandBox);
