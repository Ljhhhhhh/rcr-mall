      ...
      <transition name="topin">
        <div class="orderby-select_option" v-show="currentOpenSelect>-1">
          <template v-for="(select, index) in selectList">
            <ul :class="select.class" v-show="index===currentOpenSelect && select.name!=='品牌'" :key="select.name">
            ...
            <div class="brandScrollWrap" id="brandBox" v-show="index===currentOpenSelect && select.name==='品牌'" :key="index">
...
export default {
  ...
  methods: {
    ...
    async getBrandList() {
      ...
      console.log(brandBox.style);
      brandBox.style.maxHeight = boxHeight - searchHeight - selectHeight + 'px';
