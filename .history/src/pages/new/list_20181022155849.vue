      ...
      <transition name="topin" v-show="currentOpenSelect>-1">
        <div class="orderby-select_option" ref="options">
            ...
            <div class="brandScrollWrap" v-show="currentOpenSelect === 1">
              <h-list-view :data="selectList[1].options"></h-list-view>
            </div>
        </div>
      </transition>
    </div>
  ...
  methods: {
    ...
    async getBrandList() {
      ...
      brandRes.data.forEach(brand => {
        ...
        brandObj[brand.initial].items.push(brand.name);
      });
      ...
      document.querySelector('.brandScrollWrap').style.offsetHeight = boxHeight - searchHeight - selectHeight + 'px';
      // brandBox.style.maxHeight = boxHeight - searchHeight - selectHeight + 'px';
