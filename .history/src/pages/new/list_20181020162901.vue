<template>
  <div class="container">
    ...
    <div class="orderby-select_box van-hairline--bottom">
      <span v-for="(option, index) in selectList[currentOpenSelect].options" :key="index">
        <em>{{option.tag}}</em>
        <i class="triangle-down_icon"></i>
      </span>
      ...
      <div class="orderby-select_option">
        <ul class="orderby-select_default">
          <li v-for="(select, index) in selectList" :key="index" :class="{selected:index===select.active}">
            <span>{{select.tag}}</span>
            <van-icon color="#0198E7" name="success" v-if="index===select.active" />
          </li>
        </ul>
...
<script>
  ...
  export default {
    ...
    computed: {
      currentOptions() {
        return this.selectList[this.currentOptions].options
      }
    },
    methods: {
      selectCity() {
