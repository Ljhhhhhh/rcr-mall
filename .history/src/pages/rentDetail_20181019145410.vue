      ...
      <div class="scroll-content">
        ...
        <div class="car-info_box">
          ...
          <p class="car-info_offers">
            ...
            <label>{{carInfo.platNo}}</label>
          </p>
          <div class="car-info_price">
            <p>￥<strong>{{carInfo.payment}}</strong>/日</p><!-- <span>(市场价288元)</span> -->
            <p>￥<strong>{{(carInfo.payment/24).toFixed(2)}}</strong>/小时</p>
          </div>
        </div>
        ...
        <div class="net-info_box">
          <div class="net-info_content">
            <p class="net-info_title">
              <i></i>
              <span>{{serviceNet.name}}</span>
            </p>
            <p>{{serviceNet.addr}}</p>
            <p>营业时间：08:00-22:00</p>
            <p>服务热线：{{serviceNet.phone}}</p>
          </div>
          <div class="net-info_nevigate" v-if="serviceNet.lat && serviceNet.lon">
            <i></i>
            <span>导航</span>
          </div>
        </div>
    ...
    async getCarDetail() {
      ...
      this.confListMap.forEach(conf => {
        if (mode[conf]) {
          ...
          this.carInfo.conf.push(mode[conf]);
        }
      });
      this.serviceNet = data.dept;
    },
    async changeFollow() {
